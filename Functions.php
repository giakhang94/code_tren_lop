...
function UpdateLoai($loai){
 ...
 function GetAdminsPT ($keyword,$pageNumber,$rowNumber,&$total){
     // tính vị trí bắt đầu
     $pageNumber = ($pageNumber - 1)* $rowNumber;
     // cau lenh truy vấn
    ...
    $sql = $sql ." limit {$pageNumber},{$rowNumber}";
    // trả về các dòng hiển thị
    return Db()->query($sql);
    
 }
 // lấy danh sách tài khoản và phân trang
 function GetLoaiPT ($keyword,$pageNumber,$rowNumber,&$total){
    // tính vị trí bắt đầu
    $pageNumber = ($pageNumber - 1)* $rowNumber;
    // cau lenh truy vấn
   $sql = "SELECT * FROM `nn_loai` 
